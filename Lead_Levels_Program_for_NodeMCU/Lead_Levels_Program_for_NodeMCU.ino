...
#define WLAN_SSID  "hackBCA"
#define WLAN_PASS  "lightingthetorch"

// Configure Adafruit IO access.
#define AIO_KEY    "c59ac6e22d364c28a8738ae7261675f6"
...
Adafruit_IO_Feed lightFeed = aio.getFeed("lightLevel");
...
void setup() {
  ...
  Serial.println(F("Adafruit IO ESP8266 light sensor"));
...
void loop() {
  ...
  Serial.print(" Lead Level = ");  //print lead, keep variable as "light" in code
  Serial.print(light);
 
// send lightlevel to adafruit.io

  if (lightFeed.send(light)) {
    Serial.print(F("Wrote value to feed: ")); Serial.println(light, DEC);
  }
  else {
    Serial.println(F("Error writing value to feed!"));
  }

  // Now wait 1 seconds and read the current feed value.
  ...
  FeedData latest = lightFeed.receive();
  if (latest.isValid()) {
  ...
  // Now wait 20 more seconds and repeat.
  Serial.println(F("Waiting 20 seconds and then writing a new feed value."));
  delay(20000);
}
